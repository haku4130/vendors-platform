    ...
    >
      ...
      <div class="grid grid-cols-3 gap-4">
        ...
        <div class="space-y-1">
          ...
          <p class="font-semibold">${{ proposedCost.toLocaleString('ru-RU') }}</p>
        </div>
      </div>
      <div v-if="questions.length > 0" class="space-y-3 pt-2 border-t border-green-200">
      ...
      <div v-if="requirements.length > 0" class="space-y-3 pt-2 border-t border-green-200">
        ...
        <div class="border border-green-200 rounded-lg overflow-hidden">
          <table class="w-full text-sm">
            <thead class="bg-green-100 border-b border-green-200">
              <tr>
                <th class="text-left px-4 py-2 font-medium text-gray-600 w-1/4 border-r border-green-200">Группа</th>
                <th class="text-left px-4 py-2 font-medium text-gray-600">Требование</th>
                <th class="text-center px-4 py-2 font-medium text-gray-600 w-24 border-l border-green-200">Priority</th>
                <th class="text-center px-4 py-2 font-medium text-gray-600 w-28 border-l border-green-200">Feasibility</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in groupedFeasibilityRows" :key="row.origIndex" class="border-t border-green-100">
                <td
                  v-if="row.showGroup"
                ...
                <td class="px-4 py-2 text-center border-l border-green-200 text-gray-500">{{ row.req.priority }}</td>
                <td class="px-4 py-2 text-center border-l border-green-200 font-semibold">
...
const groupedFeasibilityRows = computed(() => {
  ...
  const rows: { req: typeof requirements.value[0]; origIndex: number; showGroup: boolean; groupRowspan: number }[] = [];
  for (const g of groupOrder) {
    const indices = grouped.get(g)!;
    indices.forEach((origIndex, j) => {
      rows.push({ req: requirements.value[origIndex], origIndex, showGroup: j === 0, groupRowspan: j === 0 ? indices.length : 0 });
    });
  }
  return rows;
});
