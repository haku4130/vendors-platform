<template>
  <div class="max-w-3xl mx-auto w-full space-y-6">
    ...
    <template v-else-if="request">
      <!-- Vendor info -->
      <div class="border border-gray-200 rounded-2xl p-6 bg-white flex items-center gap-4">
        <div
          class="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-base shrink-0"
          :style="{ backgroundColor: avatarColor(request.vendor_profile.user?.company_name ?? '') }"
        >
        ...
        <div>
          ...
          <p class="text-sm text-gray-400">{{ request.vendor_profile.user?.full_name }}</p>
        </div>
        <div v-if="request.vendor_profile.rating" class="ml-auto flex items-center gap-1">
          <UIcon name="i-lucide-star" class="w-4 h-4 text-amber-400 fill-amber-400" />
          <span class="font-semibold">{{ request.vendor_profile.rating.toFixed(1) }}</span>
        </div>
      </div>
      ...
      <div class="border border-gray-200 rounded-2xl p-6 bg-white space-y-5">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="space-y-1">
            ...
            <p class="font-semibold text-lg">{{ request.days_to_start }} дн.</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-gray-400">Длительность</p>
            <p class="font-semibold text-lg">{{ request.duration_days }} дн.</p>
          </div>
          <div class="space-y-1">
            <p class="text-xs text-gray-400">Стоимость</p>
            <p class="font-semibold text-lg">${{ request.proposed_cost?.toLocaleString('ru-RU') }}</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div
          ...
          class="space-y-1"
        >
          <p class="text-sm font-medium text-gray-700">{{ i + 1 }}. {{ projectQuestions[i] }}</p>
          <p class="text-sm bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5">{{ answer }}</p>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3">
        <template v-if="!alreadyProcessed">
          <UButton variant="outline" color="neutral" size="lg" @click="handleDeny">
            Отклонить
          </UButton>
...
import type { ProjectRequestPublicVendorFull, RequirementItem } from '~/generated/api';
...
const groupedFeasibilityRows = computed(() => {
  ...
  scores.forEach((s, i) => {
    if (!grouped.has(s.group)) { grouped.set(s.group, []); groupOrder.push(s.group); }
    grouped.get(s.group)!.push(i);
  });
  const rows: { score: typeof scores[0]; origIndex: number; showGroup: boolean; groupRowspan: number }[] = [];
  for (const g of groupOrder) {
    const indices = grouped.get(g)!;
    indices.forEach((origIndex, j) => {
      rows.push({ score: scores[origIndex], origIndex, showGroup: j === 0, groupRowspan: j === 0 ? indices.length : 0 });
    });
  }
  return rows;
});
...
async function handleAccept() {
  ...
  if (res.error) {
    toast.add({ title: 'Ошибка', description: extractErrorMessage(res.error), color: 'error' });
    return;
  }
  toast.add({ title: 'Предложение принято', description: 'Вендор получил уведомление.', color: 'success' });
...
async function handleDeny() {
  ...
  toast.add({ title: 'Предложение отклонено', description: 'Вендор получил уведомление.', color: 'success' });
